...
unix_script 是一个 macOS/Linux 脚本库，提供 61 个模块的安装、配置与管理。每个模块支持统一的子命令接口。

模块按功能分类组织在子目录中：

| 分类目录 | 说明 | 模块数 |
|----------|------|--------|
| `services/` | 服务类（Web/数据库/监控/VPN/管理面板 等） | 21 |
| `essentials/` | 装机必备（基础工具/系统初始化） | 6 |
| `dev-tools/` | 开发环境（语言/编辑器/工具链） | 15 |
...
arch:ARM64      # CPU 架构
version:1.17.0  # unix_script 自身版本
```
