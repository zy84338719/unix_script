...
- 容器工具批次①（模块 61→66，services 21→23、dev-tools 15→18）：
  ...
  - `containerd` 容器运行时（`ctr` 随附）+ `nerdctl` Docker 兼容 CLI（Linux 系统仓库缺包时回退 containerd.io，nerdctl 走 GitHub release；macOS brew 仅 CLI——宿主无法运行 Linux 容器，status 标注 cli-only）
...
### 变更
- **CI 容器矩阵换血**：移除 Ubuntu 20.04/22.04、Debian 11、CentOS 7；新增 Ubuntu 26.04；openSUSE Tumbleweed 替换为 Leap 15.6（滚动版上游源持续超时）；国产化 checkout 在 merge ref 已删时回退拉 head 分支
