...
search_processes() {
    ...
    if [[ "$search_term" =~ ^[0-9]+$ ]]; then
        ...
        if command_exists lsof; then
            ...
            netstat -an | grep "LISTEN" | grep ":$search_term " | awk '{print $9}' | cut -d'.' -f5 | sort -u > "$temp_file"
        else
            print_error "未找到合适的网络工具 (lsof/netstat)"
            return 1
        fi
    else
        # 按进程名搜索
...
show_process_details() {
    ...
    for pid in "${pids[@]}"; do
        if kill -0 "$pid" 2>/dev/null; then
            local proc_info
            proc_info=$(ps -p "$pid" -o pid,user,pcpu,comm 2>/dev/null | tail -1)
            if [[ -n "$proc_info" ]]; then
                printf "%-6d %s\n" "$index" "$proc_info"
                ((index++))
            fi
...
kill_process() {
    ...
    # 获取进程信息
    local proc_name
    proc_name=$(ps -p "$pid" -o comm 2>/dev/null | tail -1 | tr -d ' ')
